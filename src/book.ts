  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        title: "L3スイッチ",
        icon: "🔀",
        what: "VLANごとにルーティングインターフェース（SVI）を持ち、異なるVLAN同士を中継できるスイッチ。",
        reality: "ルーターのポートを部署ごとに増やさなくても、1台のL3スイッチでVLAN間の通信とインターネットへの出口を両方まかなえる。",
        inGame: "⚙設定でVLANごとにIPアドレス（SVI）とDHCPを設定できる。ルーターへのアップリンク先ゲートウェイも設定する。",
  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        title: "VLAN間ルーティング",
        icon: "🔀",
        what: "異なるVLAN同士が通信するには、どこかで「ルーティング（経路選択）」を行う機器が必要になる。",
        reality: "ルーターの物理ポートを部署の数だけ用意する代わりに、L3スイッチ1台でVLAN間の通信をまとめて処理できることが多い。",
        inGame: "L3スイッチのVLANごとのSVI（ルーティングインターフェース）をPCのゲートウェイに設定すると、そのVLANの通信はL3スイッチを経由するようになる。SVIを持たないVLANは今までどおりL2でしか通信できない。",
        example: "PC(VLAN10) → L3 Switch SVI(192.168.10.1) → VLAN20 / Internet",
        practiceMode: "settings",
      },
    ],
  },
